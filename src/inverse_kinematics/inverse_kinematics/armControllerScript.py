...
from spaceMouse import setup_spacemouse, read_spacemouse, flush_hid, flush_hid_preserve_buttons
from mathHelpers import calc_angle_between_points, get_flipped_angles, wrap_to_pi, angle_diff, Tz
from linearAlgebra import inverseKinematics, calcJointPositions, homingStep, jointLimits, collisionProtection
from pygameGraphics import compute_camera_basis, project_3d, draw_arm, draw_link, draw_cube, draw_chassis, drawGUI, initPygame
...
iAs = initialAngles
j0, j1, j2, j3 = calcJointPositions(iAs[0], iAs[1], iAs[2], iAs[3], False)
...
# Draw initial window
screen, clock, font, static_surface = initPygame()
screen.fill(colGreen)
screen.blit(static_surface, (0,0))
draw_arm(screen, j0, j1, j2, j3)
drawGUI(screen, font, point, th0, th1, th2, th3, th4, homing, buttonz, static_surface)
pygame.display.flip()
...
while running:
    ...
    if (not all(v == 0 for v in state.values())) or homing:
        ...
        if homing:
            homing, th0, th1, th2, th3, th4 = homingStep(homing, th0, th1, th2, th3, th4)
            if homing:
                screen.fill(colBlue)
            else:
                screen.fill(colGreen)
            ...
            # Compute arm joints
            try:
                th0, th1, th2, _ = inverseKinematics(point, [th0, th1, th2, th3], th4)
        ...
        # Update stuff
        if buttonz[1]:
            th0, th1, th2, th3, th4 = jointLimits(th0, th1, th2, th3, th4, False)
        else:
            th0, th1, th2, th3, th4 = jointLimits(th0, th1, th2, th3, th4, True)

        th0, th1, th2, th3, th4 = collisionProtection(th0, th1, th2, th3, th4)
        l0, l1, l2, l3 = calcJointPositions(th0, th1, th2, th3)
        point = l2

        # Draw
        drawGUI(screen, font, point, th0, th1, th2, th3, th4, homing, buttonz, static_surface)
        print(f"{time.time()-startTime:10.4f}\t\t{th0*57.3:10.4f}\t\t{th1*57.3:10.4f}\t\t{th2*57.3:10.4f}\t\t{th3*57.3:10.4f}")
        pygame.display.flip()
