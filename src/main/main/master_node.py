...
class Master(Node):
    ...
    __chassis_node_enable_publisher: (
        rclpy.publisher.Publisher
    )  # This is sent to other nodes to tell them to enable/disable the chassis
    __chassis_teensy_enable_publisher: rclpy.publisher.Publisher
    __arm_node_enable_publisher: (
        rclpy.publisher.Publisher
    )  # This is sent to other nodes to tell them to enable/disable the arm
    __arm_teensy_enable_publisher: rclpy.publisher.Publisher
    __science_node_enable_publisher: (
        rclpy.publisher.Publisher
    )  # This is sent to other nodes to tell them to enable/disable the science subsystem
    ...
    def __check_timeout(self) -> None:
        ...
        if self.__chassis_teensy_enabled:
            if self.__curr_chassis_command == self.__last_chassis_command:
                self.get_logger().info(
                    "Chassis command timeout detected, disabling chassis"
                )
                self.__chassis_node_enabled = False
                self.__chassis_teensy_enabled = False
                self.__chassis_node_enable_publisher.publish(Bool(data=False))
                self.__chassis_teensy_enable_publisher.publish(
                    self.__pack_enable_message_to_teensy(
                        CAN_MESSAGE_IDS.ENABLE_CHASSIS, False
                    )
                )
            else:
        ...
        if self.__arm_teensy_enabled:
            if self.__curr_arm_command == self.__last_arm_command:
                ...
                self.__arm_teensy_enable_publisher.publish(
                    self.__pack_enable_message_to_teensy(
                        CAN_MESSAGE_IDS.ENABLE_ARM, False
                    )
                )
            else:
        ...
        if self.__science_teensy_enabled:
            if self.__curr_science_command == self.__last_science_command:
                self.get_logger().info(
                    "Science command timeout detected, disabling science"
                )
                self.__science_node_enabled = False
                self.__science_teensy_enabled = False
                self.__science_node_enable_publisher.publish(Bool(data=False))
                self.__science_teensy_enable_publisher.publish(
                    self.__pack_enable_message_to_teensy(
                        CAN_MESSAGE_IDS.ENABLE_SCIENCE, False
                    )
                )
            else:
            ...
            try:
                ...
                if source_signal.value == SubsystemsIDs.CHASSIS:
                    ...
                    if not self.__chassis_teensy_enabled:
                        ...
                        self.__chassis_teensy_enable_publisher.publish(
                            self.__pack_enable_message_to_teensy(
                                CAN_MESSAGE_IDS.ENABLE_CHASSIS, True
                            )
                        )
                        self.get_logger().info(
                            "Chassis command received, enabling chassis teensy"
                        )
                    elif (
                        can_message.signals["enabled"].value == 1
                        and not self.__chassis_node_enabled
                    ):
                        self.__chassis_node_enabled = True
                        self.__chassis_node_enable_publisher.publish(Bool(data=True))
                        self.get_logger().info(
                            "Chassis enabled message received from teensy, enabling chassis node"
                        )
                    ...
                    if not self.__arm_teensy_enabled:
                        ...
                        self.__arm_teensy_enable_publisher.publish(
                            self.__pack_enable_message_to_teensy(
                                CAN_MESSAGE_IDS.ENABLE_ARM, True
                            )
                        )
                        self.get_logger().info(
                            "Arm command received, enabling arm teensy"
                        )
                    elif (
                        can_message.signals["enabled"].value == 1
                        and not self.__arm_node_enabled
                    ):
                        self.__arm_node_enabled = True
                        self.__arm_node_enable_publisher.publish(Bool(data=True))
                        self.get_logger().info(
                            "Arm enabled message received from teensy, enabling arm node"
                        )
                    ...
                    if not self.__science_teensy_enabled:
                        ...
                        self.__science_teensy_enable_publisher.publish(
                            self.__pack_enable_message_to_teensy(
                                CAN_MESSAGE_IDS.ENABLE_SCIENCE, True
                            )
                        )
                        self.get_logger().info(
                            "Science command received, enabling science teensy"
                        )
                    elif (
                        can_message.signals["enabled"].value == 1
                        and not self.__science_node_enabled
                    ):
                        self.__science_node_enabled = True
                        self.__science_node_enable_publisher.publish(Bool(data=True))
                        self.get_logger().info(
                            "Science enabled message received from teensy, enabling science node"
                        )
